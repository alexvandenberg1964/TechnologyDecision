...
### 1.2 "Waarom zijn er alleen harde constraints richting High-Code — nooit richting Low-Code of tegen Hybrid als zodanig?" — 🟡 deels geadresseerd tijdens deze review

**Geverifieerd (oorspronkelijk):** alle 5 KO-regels (`no_ui_full`, `no_ui_qs`, `extreme_latency`, `extreme_elasticity`, `zero_trust`) hadden `forceKey:'hc'`. Er bestond geen enkele regel die Low-Code afdwingt of High-Code blokkeert. De 2 `block`-regels en de ene `warn`-regel gelden uitsluitend als Hybrid wint. *(Update: `no_ui_full` en `no_ui_qs` zijn sindsdien allebei verwijderd — zie 1.6 en het bijbehorende Quick Scan-vervolgpunt — dus er zijn nu nog 3 KO-regels, `extreme_latency`/`extreme_elasticity`/`zero_trust`, nog steeds allemaal `forceKey:'hc'`. De kernvraag van deze bevinding staat dus nog steeds.)*
...
**Doorgevoerde wijziging (het "notion"-deel, niet de regel-asymmetrie zelf):**

- De "Low-Code is de strategische default"-aanname is verwijderd uit de disclaimer in de sidebar — die zin bestaat niet meer. De disclaimer legt nu uit dat de standaardgewichten algemene richtlijnen zijn, geen platformvoorkeur, en dat een exacte gelijke stand beide platforms als gelijkwaardige opties toont.
- De `vendor`-vraagoptie "Not a concern — Low-Code is the PostNL strategic platform" is hernoemd naar "Not a concern — lock-in risk is acceptable for this application" — dezelfde onderliggende aanname stond ook daar impliciet in de tekst. Score-waarde ongewijzigd, alleen de formulering.
- **Bijvangst — tie-handling gecentraliseerd en écht correct gemaakt:** bij het doorlichten van "wat gebeurt er bij een exacte gelijke stand" bleek de tie-detectielogica op 4 plekken apart en inconsistent geïmplementeerd te zijn — en de Blind Assessment-export (`buildBlindResult()`) had helemaal géén tie-detectie, dus rapporteerde bij een exacte gelijke stand stilzwijgend Low-Code als "de aanbeveling" (puur door array-volgorde). Dit is nu gecentraliseerd in `determineWinner()` (`isTie`/`tied`), door alle 4 plekken (sidebar, finish-scherm, export, blind-assessment-resultaat) hergebruikt, en de Organisational-Readiness-"ready"-vlag is nu tri-state (true/false/**null**) zodat een gelijke stand niet ten onrechte als "GAP" wordt getoond. Regressietests toegevoegd.
  **Nog open — bewust niet zelf besloten:** de kernvraag van deze bevinding (waarom bestaan er geen harde constraints die Low-Code afdwingen of High-Code blokkeren) is een architectuurbeslissing die het architectuurteam expliciet moet maken, niet iets wat stilzwijgend in code opgelost hoort te worden — dat blijft dus openstaan zoals het verbetervoorstel zelf al aangaf.
...
- Toegankelijk via dezelfde tool-koppeling als het bestaande calibratieregister (sidebar-knop "📖 Scoring calibration & rationale").
- **Update — nu ook inline in de tool zelf (was in eerste instantie alleen als los document):** elke Full Assessment-vraag heeft een "🗂️ Examples"-badge (verborgen tijdens Blind Assessment, net als elke andere scoring-hint) die een popup opent met de voorbeeldapplicatie per antwoordoptie van die specifieke vraag — precies op het moment dat een assessor een keuze maakt, niet pas als hij achteraf `CALIBRATION.md` erbij zou pakken. De inhoud is een handmatig bijgehouden kopie van `CALIBRATION.md` §9 (`QUESTION_EXAMPLES` in de code) — twee plekken die synchroon gehouden moeten worden, getrackt als nieuw openstaand punt in `CALIBRATION.md` §10.
  **Nog open:** (1) de 4 opties die nog maar op één testset-app leunen (`ui_complexity` "specialistische visualisatie", `integration` "weinig standaardintegraties", `cloud` "on-premise", `change_rate` "snelle initiële levering") zouden een tweede, onafhankelijke voorbeeld verdienen; (2) alle 24 Organisational-Readiness-voorbeelden zijn geconstrueerd, niet uit echte casussen — dat is de grootste resterende kloof; (3) de jaarlijkse kalibratiesessie zelf (architecten die onafhankelijk dezelfde casus scoren) is nog niet gepland; (4) de dubbele opslag van voorbeelden (document + code) is een nieuw, zelf geïntroduceerd onderhoudsrisico — zie `CALIBRATION.md` §10 voor de volledige lijst openstaande punten. Dit is dus een inhoudelijk complete, maar nog niet door een panel gevalideerde, eerste versie — vandaar 🟡 in plaats van ✅.
...
| **🟡 Deels gedaan** | `CALIBRATION.md` §9 toegevoegd — voorbeeldapplicaties per antwoordoptie (55/58 Technology-Fit-opties uit de testset, 24 geconstrueerde Organisational-Readiness-voorbeelden), nu ook inline in de tool via een "🗂️ Examples"-popup per vraag; jaarlijkse kalibratiesessie nog niet gepland                                                                                     | 3.2                          |
| **🟡 Deels gedaan** | "Low-Code is de strategische default"-notie verwijderd uit disclaimer en `vendor`-optie; tie-handling gecentraliseerd zodat een gelijke stand beide platforms als optie toont i.p.v. stilzwijgend Low-Code. Kern-asymmetrie in `HARD_RULES` (alleen richting High-Code) blijft een architectuurteam-beslissing | 1.2                          |
